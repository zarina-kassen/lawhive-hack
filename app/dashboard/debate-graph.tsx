...
import { BadgePoundSterling, CircleDot, Scale, Split } from "lucide-react";
...
export default function DebateGraph({ result }: { result: SimulationResult }) {
  const [selectedDebateId, setSelectedDebateId] = useState(result.debates[0]?.id ?? "");

  // Falls back to the first debate if the selected id isn't in the current result.
